...
START_TEST(test_calc_complements_valid) {
  test_start("test_calc_complements_valid");
  matrix_t A, result;
  ...
  test_end("test_calc_complements_valid");
}
END_TEST

START_TEST(test_calc_complements_2x2) {
  test_start("test_calc_complements_2x2");
  matrix_t A, result;
  ...
  s21_remove_matrix(&A);
  s21_remove_matrix(&result);
  test_end("test_calc_complements_2x2");
}
END_TEST

START_TEST(test_calc_complements_non_square) {
  test_start("test_calc_complements_non_square");
  matrix_t A, result;
...
START_TEST(test_calc_complements_null_result) {
  ...
  s21_remove_matrix(&A);
  test_end("test_calc_complements_null_result");
}
END_TEST

START_TEST(test_calc_complements_1x1) {
  test_start("test_calc_complements_1x1");
  matrix_t A, result;
  ...
  s21_remove_matrix(&A);
  test_end("test_calc_complements_1x1");
}
END_TEST
